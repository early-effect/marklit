...
/** Scala 2.13-only API: a sealed-trait + case-objects encoding of the same
  ...
  *
  * Lives in `src/main/scala-2.13/`, which sbt only includes on the 2.13 build.
  * Calls in the markdown look identical (`Color.Red`, `c.code`) regardless of
  * which underlying encoding is on the classpath.
  */
